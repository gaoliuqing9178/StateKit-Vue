...
当前代码库已经定义并实现了 19 个 Block，按 7 个类别组织：

| 类别 | 数量 | 代表场景 |
| --- | --- | --- |
| `empty` | 3 | 空集合、空搜索、工作区内尚未创建首个项目 |
| `onboarding` | 1 | 首次创建工作区 |
...
- onboarding 已经作为独立类别试点落地，当前先用 `onboarding-workspace` 作为公开入口验证这条产品边界。
- `first-project` 仍保留在 `empty` 类别中，用来表达"工作区已经存在，但还没有第一个项目"的兼容过渡场景。
- 当前元数据中标记为 `launch` 的优先 Block 有 7 个：`empty-search`、`onboarding-workspace`、`first-project`、`page-error`、`no-permission`、`upgrade-plan`、`task-success`。

### 2. 平台范围

- V1 只正式面向 Vue 3。
- API 已收敛到按类别统一的组件入口，例如 `EmptyState`、`OnboardingState`、`PermissionState`、`UpgradeState`；19 个 preset recipe 继续保留作为默认场景参考。
